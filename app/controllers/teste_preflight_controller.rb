...
class PreflightController < ApplicationController
	...
	def index
		...
		#identifica a necessidade de encaminhar o arquivo para a baixa
  		if ((sizeMM[0] < largura || sizeMM[1] < altura) && rotate == false)
  			@baixa = true
  		else
  			@baixa = false
  		end
		...
		begin
			...
			File.open("erro.txt", "a") do |arquivo_temporario|
				arquivo_temporario << "Erro ao validar " + fullFilePath + " \n"
			end
		end
		...
		#adicional de verniz
		if ((lado == "2" && qtdCores == "4x1") || lado == "3" || lado == "4")
	...
	def newThumbnail(filepath, filename, largura, altura, rotate, lado, fileSize, qtdCores)	
		...
		tiff_img = filepath + filename.sub(/.pdf/) { ".tif" }
		...
		begin
=begin			
			if (((lado == "2" && qtdCores == "4x1") || lado == "3" || lado == "4") && @baixa == false)
				temp_pdf = filepath + filename.sub(/.pdf/) { "_temp.pdf" }
				system('gs -sDEVICE=pdfwrite -sProcessColorModel=DeviceGray -sColorConversionStrategy=Gray -dNOPAUSE -dBATCH -q -dSAFER -dAutoRotatePages=/None -sOutputFile=' + "#{temp_pdf} #{pdf}")
				system('rm ' + "#{pdf}")
...
=end
			...
			File.open("erro_gera_thumb.txt", "a") do |arquivo_temporario|
				arquivo_temporario << "Erro ao tentar escrever a thumb \n"
			end
			false
		end
		...
		for i in 0..objects.size
			begin
				if (objects[i][:OPM] == 1) && (objects[i][:op] == true || objects[i][:OP] == true)
					@erros << Preflight::Issue.new(0, "colorview", "Cor sobreposta encontrada", objects, objects[i])
					@overprint = 1
					break
				else
					@overprint = 0
				...
				begin
					...
					begin
						...
						if img.include? ("ImageC")
							@erros << Preflight::Issue.new(0, "colorview", "Identificamos que #{tipoArquivo} contém imagens coloridas. <br/>Ressaltamos que somente tons de cinza ou o canal K da paleta CMYK podem ser impressos.", objects, colorIssue)
							break
						end
					rescue
						img = stream['ProcSet']
						if img.include? ("ImageC")
							@erros << Preflight::Issue.new(0, "colorview", "Identificamos que o VERSO contém imagens coloridas. <br/>Ressaltamos que somente tons de cinza ou o canal K da paleta CMYK podem ser impressos.", objects, colorIssue)
							break
						end
					end
	...
	def processFileColors(fullPathFilename, qtdCores, lado)
		...
		rgbIssue = {Erro: "Arquivo possui imagem com cores RGB"}
			...
			begin
				...
				#	@erros << Preflight::Issue.new(1, "colorview", "Arquivo possui imagem com cores RGB.", reader, rgbIssue)
				...
				if file_specs.include?("Colorspace: Gray")
					...
					#	@erros << Preflight::Issue.new(1, "colorview", "Este arquivo não pode conter imagens coloridas.", reader, cmykIssue)
		...
		for i in 0..objects.size
			...
			begin
				...
				if type.include? ("Font")
					@erros << Preflight::Issue.new(0, "pdfview", "O Arquivo PDF possui fontes não convertidas em curvas. Isso pode causar alterações no resultado final do seu impresso.", objects, fontIssue)
